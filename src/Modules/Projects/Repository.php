...
final class Repository
{
    ...
    public function getProjectSummary(int $projectId): ?array
    {
        ...
        $project['members_count'] = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$this->table('members')} WHERE project_id = %d",
            $projectId
        ));
        $project['tasks_count'] = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$this->table('tasks')} WHERE project_id = %d AND status <> 'archived'",
            $projectId
        ));
        $project['open_tasks_count'] = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$this->table('tasks')} WHERE project_id = %d AND status = 'open'",
            $projectId
        ));
        $project['done_tasks_count'] = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$this->table('tasks')} WHERE project_id = %d AND status = 'done'",
            $projectId
        ));
        $project['deliverables_count'] = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$this->table('deliverables')} WHERE project_id = %d",
            $projectId
        ));
        $project['validated_deliverables_count'] = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$this->table('deliverables')} WHERE project_id = %d AND status = 'validated'",
            $projectId
        ));
        $project['overdue_deliverables_count'] = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$this->table('deliverables')} WHERE project_id = %d AND status NOT IN ('validated', 'rejected') AND due_date IS NOT NULL AND due_date < %s",
            $projectId,
            current_time('Y-m-d')
        ));
        $project['last_log_at'] = (string) $wpdb->get_var($wpdb->prepare(
            "SELECT created_at FROM {$this->table('logs')} WHERE project_id = %d ORDER BY created_at DESC, id DESC LIMIT 1",
            $projectId
        ));
        $project['last_evidence_at'] = (string) $wpdb->get_var($wpdb->prepare(
            "SELECT created_at FROM {$this->table('evidence')} WHERE project_id = %d ORDER BY created_at DESC, id DESC LIMIT 1",
            $projectId
        ));
    ...
    public function getBoard(int $projectId): array
    {
        ...
        $columns = $wpdb->get_results($wpdb->prepare(
            "SELECT * FROM {$this->table('columns')} WHERE project_id = %d ORDER BY position ASC, id ASC",
            $projectId
        ), ARRAY_A) ?: [];
    ...
    private function getChecklistForTasks(array $taskIds): array
    {
        ...
        $rows = $wpdb->get_results($wpdb->prepare(
            "SELECT * FROM {$this->table('checklist_items')} WHERE task_id IN ({$placeholders}) ORDER BY task_id ASC, position ASC, id ASC",
            $taskIds
        ), ARRAY_A) ?: [];
    ...
    public function getEvidence(int $projectId): array
    {
        ...
        $rows = $wpdb->get_results($wpdb->prepare(
            "SELECT e.*, u.display_name, d.title AS deliverable_title, t.title AS task_title
             ...
             LEFT JOIN {$this->table('deliverables')} d ON d.id = e.deliverable_id
             LEFT JOIN {$this->table('tasks')} t ON t.id = e.task_id
    ...
    public function getEvidenceItem(int $evidenceId): ?array
    {
        ...
        $row = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM {$this->table('evidence')} WHERE id = %d LIMIT 1",
            $evidenceId
        ), ARRAY_A);
    ...
    public function updateEvidence(int $evidenceId, array $data): bool
    {
        ...
        if (array_key_exists('evidence_type', $data)) {
            $updates['evidence_type'] = self::cleanEvidenceType($data['evidence_type']);
            $formats[] = '%s';
        }

        if (array_key_exists('url', $data)) {
            $updates['url'] = self::cleanUrl($data['url']);
            $formats[] = '%s';
        }
    ...
    {
        $id = self::cleanNullableId($value);
        if ($id === null) {
            return null;
        }

        $deliverable = $this->getDeliverable($id);

        return $deliverable && (int) $deliverable['project_id'] === $projectId ? $id : null;
    }
