...
check(CodeWorkspace::resource($code, $state)['initial_content'] === $code['content'], 'Reset exposes original snippet, never expected correction');
[$resetState] = CodeWorkspace::save($ticket, $state, $code, CodeWorkspace::resource($code, $state)['initial_content']);
check($resetState['code_edits'][$code['id']] === $code['content'] && !isset($resetState['tests']['verify']), 'Saving initial code restores original and invalidates successful test');
